...
luminex_dir <- 
  str_c(VIBRANT_dropbox_dir, "15_VIBRANT Luminex/VMRC VIBRANT LUMINEX Results_ONGOING/Harmonised Batches")
...
:::callout-note
Not clear what "softcup blanks" are - maybe worth checking with the team again.
:::
...
manifest_file <- 
  list.files(
    luminex_dir, 
    full.names = TRUE, 
    pattern = "^Combined batches_LS_LL_20250507.xlsx"
  )
...
manifest_col <- 
  c(
    "BATCH", 
    "LOCATION", "MANIFEST_SAMPLE_VCODE", "PATIENT", "VISIT CODE", "VISIIT", "SAMPLED_DATE",
    "Weight", "Volume", "ALIQUOT VOLUME...14", "DILUTION/200ul TUBE...15", 
    "VOLUME ADDED FOR ASSAY...16", "VOLUME AFTER DILUTION...17", "DILUTION OF ORIGINAL SAMPLE", 
...
manifest <- 
  ...
  dplyr::rename(
    visit = visiit,
    participant = patient,
    aliquot_volume = aliquot_volume_14,
    dilution_200ul_tube = dilution_200ul_tube_15,
    volume_added_for_assay = volume_added_for_assay_16,
    volume_after_dilution = volume_after_dilution_17,
...
expected_samples <- 
  read_csv(
    str_c(VIBRANT_dropbox_dir, 
          "99_VIBRANT_specimen_collection_CRF_data/CRF35_weekly_specimen_collection.csv")
    )
...
matched <- 
  full_join(
    expected_samples |> 
      mutate(
        in_CRF_35 = TRUE,
        visit_code = visit_code |> factor(), 
        pid = pid |> as.character()
      ) |> select(-randomized),
    colData(raw) |> 
      as.data.frame() |> 
      as_tibble() |> 
      select(location, participant, visit_code, visit) |>
      filter(!is.na(participant), !is.na(visit_code), !str_detect(participant, "XX")) |> 
      distinct() |> 
      mutate(
        has_luminex_data = TRUE,
        visit_code = visit_code |> factor(), 
        pid = participant |> str_remove("68")
        ),
    by = join_by(location, pid, visit_code)
  ) |> 
  left_join(
    expected_samples |> 
  ...
  mutate(
    ...
    in_CRF_35 = ifelse(in_CRF_35, "Yes", "No"),
    pid_in_CRF_35 = ifelse(pid_in_CRF_35, "Yes", "No"),
    randomized = ifelse(randomized, "Yes", "No"),
    softcup_collected = 
      case_when(
        (softcup == "Checked") & !is.na(softcup_time_placed) & (softcup_time_placed != "not done") ~ "Yes",
        (softcup == "Checked") & is.na(softcup_time_placed) ~ "Maybe",
        (softcup == "Checked") & (softcup_time_placed == "not done") ~ "Maybe",
        (softcup == "Unchecked") & !is.na(softcup_time_placed) ~ "Maybe",
        (softcup == "Unchecked") & is.na(softcup_time_placed) ~ "No",
        (softcup == "not done") ~ "No",
        is.na(softcup) ~ "Not in CRF35",
        TRUE ~ "???"
      ) |> factor(levels = c("Yes", "Maybe", "No", "Not in CRF35", "???"))
  )
...
All Luminex data participant IDs match those from the CRF35 data.
...
matched |> 
  filter(((randomized == "Yes") & (softcup_collected != "No")) | (has_luminex_data == "Yes")) |> 
  ggplot() +
  aes(x = visit_code, y = pid, shape = softcup_collected, col = has_luminex_data) +
  geom_point() +
...
There are only 5 participants for whom we are missing one softcup sample, and 3 participants that do not have matching sample ids in the CRF35 data.
...
matched |> 
  filter(((randomized == "Yes") & (softcup_collected != "No")) | (has_luminex_data == "Yes")) |> 
  filter(((has_luminex_data == "No")) | (softcup_collected == "Not in CRF35")) |> #  
  select(pid, visit_code, softcup_collected, has_luminex_data, location) |>
  arrange(location, pid) |> 
  group_by(pid) |> 
  gt(row_group_as_column = TRUE) 

```

For the 3 participants that had Luminex data not expected according to CRF35, it looks like the visit was miscoded (11 or 1 instead of 10, 1000, or 2120). 

There remain two participants for whom CRF35 claims that softcups were collected but for whom we do not have any data.
...
us_weight_data <- 
  read_xlsx(
    str_c(luminex_dir, "/US manifests/VIBRANT_LUMINEX_US MANIFEST all boxes.xlsx"),
    range = "A1:N210"
  )
...
```
:::callout-important
There is no "Bead count" sheet in the excel file for plate 2 of batch 1
:::
...
:::callout-caution
One well appear to have a very low bead count; we will flag this sample as "failed" in the section @sec-filtering below. 
:::
...
purrr::walk(
  ...
  \(x) {
    ...
    g_unadj_conc <- 
      tmp |> 
      ggplot() +
      aes(x = unadj_conc |> log2(), y = plate_name |> factor() |> fct_rev(), col = plate_name, fill = plate_name) +
      geom_boxplot(alpha = 0.5, outlier.size = 0.5) +
      geom_point(data = biol_ctrl, col = "black", size = 1) +
      guides(col = "none", fill = "none") +
      ylab("Plate") +
...
analytes_to_exclude |> 
  ggplot() +
  aes(y = .feature, x = frac_OOR, fill = exclude) +
  geom_col() +
  ylab("") +
  scale_fill_manual(
    "", 
    ...
    labels = c("Excluded analyte", "Included analyte")
  ) +
...
# analytes_to_exclude <- analytes_to_exclude |> filter(exclude) |> pull(.feature) |> as.character() |> sort()

# raw <- 
#   raw |> 
#   mutate(analyte_to_exclude = (.feature %in% analytes_to_exclude))

tmp <- 
  raw |> 
  as_tibble() |> 
  left_join(
    analytes_to_exclude |> select(.feature, frac_OOR, exclude),
    by = join_by(.feature)
  )
...
raw <- 
  ...
  mutate(
    ...
    analyte_to_exclude = tmp$exclude
    )
...
failed_samples <- 
  raw |> 
  as_tibble() |> 
  filter(!analyte_to_exclude, sample_type == "Sample") |> 
  group_by(.sample, sample_id, participant, visit_code, visit, plate_name) |> 
  summarize(
    n_OOR = sum(value_type != "Observed concentration"), 
    n_analytes = n(), 
...
failed_samples |> 
  ...
  ggplot() +
  aes(x = sample_id, y = n_OOR, col = plate_name) + 
...
Another likely justification for these extreme OOR values to be technical fail and not low biological signal is that most participants have at most of their samples for which the number of out-of-range analytes is low:
...
We still see some "participant-effect" (maybe due to the properties of their mucus), but there a no "failed" samples for which we do not have a replicate.

:::callout-note
We flag samples with less than 100 total bead count and more than 50% OOR analytes as "failed".
:::
...
tmp <- 
  ...
  left_join(
    raw |> 
      as_tibble() |> 
      filter(!analyte_to_exclude) |> 
      group_by(.sample, sample_type) |> 
      summarize(
        total_bead_count = ifelse(all(is.na(bead_count)), Inf, sum(bead_count, na.rm = TRUE)),
      ...
      mutate(
        failed_sample = 
          case_when(
            (sample_type == "Sample") & ((n_OOR/n > 0.5) | (total_bead_count < 100))  ~ TRUE,
            TRUE ~ FALSE
          )
      )
  )

raw$failed_sample <- tmp$failed_sample
raw$n_OOR <- tmp$n_OOR
raw$total_bead_count <- tmp$total_bead_count
rm(tmp, failed_samples)

```

> TODO: check if any of the failed sample did not have a replicate
...
raw |> 
  ...
  ggplot() +
  aes(x = plate_name, y = conc_log2, col = sample_type) +
...
g_12 <- 
  plot_pca_scores(pca_res, complete |> filter(sample_type == "Sample", !failed_sample), axes = 1:2, col_by = "plate_name") +
  stat_ellipse()

g_23 <- 
  plot_pca_scores(pca_res, complete |> filter(sample_type == "Sample", !failed_sample), axes = 2:3, col_by = "plate_name") +
  stat_ellipse()

g_45 <- 
  plot_pca_scores(pca_res, complete |> filter(sample_type == "Sample", !failed_sample), axes = 4:5, col_by = "plate_name") +
  stat_ellipse()
...
scores_long |> 
  ...
  ggplot(aes(x = dilution_new, y = value, col = n_OOR)) +
...
- `sd_log2_obs_conc_imp`: the standard deviation of the log2 of the imputed concentrations across replicates;


- `obs_conc_log2`: log2 of the median concentrations;

- `obs_conc_imp_log2`: log2 of the median imputed concentrations;
...
aggregate_se <- function(se){
  ...
  excluded_analytes <- se@NAMES[rowData(se)$analyte_to_exclude] # we'll add them to the metadata
  se <- se[!rowData(se)$analyte_to_exclude, ] 
  ...
  aggregate_assay <- function(se, assay_name){
    se |> 
      ...
      summarize(
        values = median(values[!failed_sample], na.rm = TRUE),
        .groups = "drop"
      ) |> 
  ...
  sd_log2_conc_assay <- 
    ...
    group_by(uid, .feature) |> 
    summarize(
      sd = conc_log2[!failed_sample] |> sd(na.rm = TRUE),
      .groups = "drop"
    ) |> 
  ...
  coldata_agg <- 
    ...
    group_by(uid) |> 
    summarize(
      n_wells = sum(!failed_sample),
      wells = str_c(well_id[!failed_sample], collapse = ", "),
      excl_wells = str_c(well_id[failed_sample], collapse = ", "),
      .groups = "drop"
    ) 
    ...
    SummarizedExperiment(
      assays = 
        list(
          ...
          sd_log2_obs_conc_imp = sd_log2_conc_assay[f, s] |> set_rownames(new_f),
          obs_conc_log2 = obs_conc_assay[f, s] |> log2() |> set_rownames(new_f),
          obs_conc_imp_log2 = obs_conc_imp_assay[f, s] |> log2() |> set_rownames(new_f)
          ),
      colData = coldata[s,],
...
selection |> 
  ggplot() +
  aes(x = visit |> factor(), y = obs_conc_imp_log2, col = pid) +
...
saveRDS(
  raw, 
  str_c(
    get_output_dir(data_source = "real"),  
