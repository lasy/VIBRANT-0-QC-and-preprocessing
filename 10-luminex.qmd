...
- 068200087 "0011" visit is indeed a screening visit and this participant has a missing sample for visit 2120, but the screening visit code should be 0010 to match that of other assays at that screening visit. **We fix the visit code for that participant**
...
- 068200247 has a visit "mismatch": both 0001 (expected) and 0000 (found) are screening visits. Since the dates for these visits are the same, and since all other assays have visit code 0000, we leave it as is
...
raw <- 
  ...
  mutate(
    visit_code = 
      case_when(
        pid == "068200087" & visit_code == "0011" ~ "0010",
        pid == "068200225" & visit_code == "0001" ~ "1000",
        # pid == "068200247" & visit_code == "0000" ~ "0001", # all other assays have visit code 0000
...
Now that `visit_code`s have been fixed, we can add the `uid` (unique VIBRANT cross-assay clinical sample identifier).

```{r}

raw <- raw |> mutate(uid = str_c(pid, "_", visit_code))
...
us_weight_data |> 
  ggplot() +
  aes(x = softcup_weight, y = softcup_volume_with_saline_imp - volume_of_saline_added) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  geom_point(alpha = 0.5) +
  ylab("Softcup volume (without saline)\n(imputed)") +
  xlab("Softcup weight") +
...
We note that that there are no clinical samples with failed samples for which we didn't have valid replicates:
...
raw@colData |> 
  ...
  summarize(
    n_replicates = n(),
    n_failed = sum(exclude_sample),
    reasons_failed = paste(exclude_sample_reason[exclude_sample] |> unique() |> sort(), collapse = ", "),
    n_valid = sum(!exclude_sample)
  ) |> 
  filter(n_failed > 0) |> 
  arrange(n_replicates) |> 
  gt(caption = "Samples for which there were failed wells")

```

```{r}
n_failed <- raw$exclude_sample |> sum()
n_wells_samples <- (raw$sample_type == "Sample") |> sum()
```


In total, there are only `r n_failed` out of `r n_wells_samples` wells that "failed" (*i.e.*, `r round(n_failed/n_wells_samples*100, 1)` % of the wells).
...
g_12 / (g_23 | g_45) + plot_layout(guides = "collect", heights = c(1, 1.5))
...
aggregate_se <- function(se){
  ...
  # We keep all analytes, but keep their exlusion flags and the reasons for exclusion
...
We check again that the failed samples did not lead to the exclusion of actual samples (i.e., that there were always at least one "good" replicate for each sample):
