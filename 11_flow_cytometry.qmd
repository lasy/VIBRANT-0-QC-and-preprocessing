...
flow_dir <- str_c(get_VIBRANT_Dropbox_dir(), "11_VIBRANT_Flow/")
output_dir <- 
  str_c(
    # get_VIBRANT_Dropbox_dir(), "90_VIBRANT_consolidated_data/", 
    get_data_dir(),
    "01 Preprocessed and QCed/06 Flow cytometry tmp files/"
    )
...
For MGH samples, we need to load the list of samples along with the machine that was used for each sample. This is provided in the `MGH_sample_list_with_machine.csv` file, which contains the expected visit codes and sample IDs.

```{r}

mgh_samples <- read_csv(file = str_c(get_data_dir(), "00 Raw/06 Flow cytometry/MGH_sample_list_with_machine.csv"))
mgh_samples |> head()
...
gate_mapping <- 
  read.csv(
    str_c(get_data_dir(), "00 Raw/06 Flow cytometry/gate_mapping_completed.csv")
  )
...
load("/Users/laurasymul/OneDrive - UCL/Academia/Research/VIBRANT clinical data UCLouvain/Data processed/2025-05-29/crf_clean.Rdata", verbose = TRUE)


load("/Users/laurasymul/OneDrive - UCL/Academia/Research/VIBRANT clinical data UCLouvain/Data processed/2025-05-29/crf_plates_dictionary.Rdata", verbose = TRUE)
