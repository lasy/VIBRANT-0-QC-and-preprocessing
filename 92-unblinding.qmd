...
The purpose of this document is to unblind the study arms. This means that we will replace the `arm` column in the `mae@colData` with the actual treatment arms based on the randomization codes. These codes were stored in separate files, prepared completely independently from the study clinical and analytical staff. 

They were sent by email after the clinical data and assay data had been checked, and formatted for downstream analyses.

This document loads these files and add the study arm to the MAE via merging by randomization code.
...
The unblided `MAE` object is saved to disk as an `.rds` file that can be loaded for downstream analyses.
