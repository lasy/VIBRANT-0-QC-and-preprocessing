...
technical_metadata <- 
  technical_metadata |> 
  mutate(
    sample_type = sample_type |> str_replace("lS", "l s") |> str_replace("b\\+C", "b \\+ C"),
    control_type = 
      case_when(
  ...
  relocate(control_type, .after = sample_type) 

technical_metadata |> dplyr::count(sample_type, control_type) |> gt()
...
dictionary <- 
  ...
  bind_rows(
    tibble(
      name = "control_type",
      description = "Control type (Mock 1, Mock 2, etc.)"
    )
  )
```
...
dictionary <- 
  ...
  bind_rows(
    tibble(
      name = "pid",
      description = "Harmonized participant ID"
    )
  )
```
  ...
  mutate(
    category = 
      case_when(
        ...
        !has_mg_data & in_CRF_35 & (self_collected_swabs > 0) ~ "Sample not sequenced",
...
matched |> 
  ...
  dplyr::count(randomized, category, name = "n samples") |> 
  arrange(randomized, -`n samples`) |> 
  dplyr::rename(
...
plot_sample_categories(matched |> filter(randomized | category == "Unexpected participant ID")) + 
  ggtitle("Randomized participants and unexpected participant IDs")
...
matched |> 
  filter(randomized & (category == "Sample not sequenced")) |> 
...
technical_metadata_agg <- 
  technical_metadata_agg |> 
  group_by(pid, visit_code) |> mutate(n = n()) |> ungroup() |> 
  mutate(
    expected = 
      (sample_type != "Clinical sample") |
      (
        (sample_category %in% c("Expected sample", "Sequenced sample from unrandomized participants")) & 
          (n == 1)
      )
  ) |>
  # arrange(pid, visit_code, sample_category, sample_type, swab_barcode) |>
  # group_by(expected) |> 
  # mutate(i = row_number() |> as.character() |> str_pad(width = 2, pad = "0")) |> 
  # ungroup() |>
  mutate(
    uid = 
      case_when(
        (sample_type == "Clinical sample") & expected ~ str_c(pid, "_", visit_code),
        TRUE ~ swab_barcode
      ),
  ) |> 
  relocate(uid, .before = mg_sample_id) |> 
  select(-n, -expected)
...
mg_to_SE <- function(counts, counts_corr, relabs, technical_metadata_agg, dictionary, VIRGO2_taxonomic_table, LBP_strain_info) {
  ...
  assay_counts <- 
    counts |> 
    mutate(mg_sample_id = sampleID) |> 
    dplyr::select(-c(sampleID, CST, subCST, score)) |> 
    as.data.frame() |> 
    column_to_rownames("mg_sample_id") |> 
    drop_na() |> 
    t() |> 
    pmax(0)
  ...
  se_coldata <- 
    ...
    left_join(
      counts |> 
        mutate(mg_sample_id = sampleID) |> 
        select(mg_sample_id, CST, subCST, score) |>
        dplyr::rename(
          cst = CST,
          sub_cst = subCST,
      ...
      by = join_by(mg_sample_id)
    ) |> 
    select(uid, mg_sample_id, sample_type, control_type, total_non_human_reads, cst, sub_cst, valencia_score, everything()) |> 
  ...
  ordered_mg_sample_ids <- se_coldata$mg_sample_id
  assay_counts <- assay_counts[, ordered_mg_sample_ids] |> set_colnames(se_coldata$uid)
  assay_counts_corr <- assay_counts_corr[, ordered_mg_sample_ids] |> set_colnames(se_coldata$uid)
  assay_relative_ab <- assay_relative_ab[, ordered_mg_sample_ids] |> set_colnames(se_coldata$uid)
...
vegan::diversity(SE_mg |> assay("rel_abs"), index = "shannon", MARGIN = 2) |> 
  ...
  left_join(SE_mg |> colData() |> as_tibble(), by = join_by(uid)) |> 
  ggplot() +
...
summarized_rel_abs |> 
  ...
  facet_grid(. ~ sample_type + sample_category, scales = "free", space = "free") +
  scale_fill_manual(
    "", 
...
summarized_rel_abs |> 
  filter(sample_type == "Clinical sample", sample_category != "Unexpected participant ID") |> # 
  ggplot() +
...
summarized_rel_abs |> 
  filter(sample_type == "Clinical sample", sample_category == "Expected sample") |> # 
  ggplot() +
  aes(x = pid, y = rel_abs, fill = taxa) +
  geom_col() +
  scale_x_discrete("Participant") + # , breaks = NULL) +
...
sample_type_colors <- c("gray", "dodgerblue", "black", "purple", "red")
