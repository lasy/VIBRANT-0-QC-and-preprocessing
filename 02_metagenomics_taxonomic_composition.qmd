...
technical_metadata_agg <- 
  ...
  summarise(
    ...
    across(cols_to_aggregate, ~ unique(.) |> na.omit() |> str_c(collapse = "; "))
  ) |> 
...
technical_metadata_agg <- 
  ...
  distinct() |> 
  left_join(technical_metadata_agg, by = join_by(mg_uid))
  ...
  mutate(
    category = 
      case_when(
        ...
        !in_mg_manifest & in_CRF_35 & (self_collected_swabs > 0) & randomized ~ "Sample from a randomized participant not in mg manifest",
        !has_mg_data & in_CRF_35 & (self_collected_swabs > 0) & randomized ~ "Sample from a randomized participant not sequenced",
        !in_mg_manifest & in_CRF_35 & (self_collected_swabs > 0) & (!randomized) ~ "Sample from a non-randomized participant not in mg manifest",
...
matched |> 
  dplyr::filter(category == "Sequenced sample not listed in CRF35") |> 
  gt()
