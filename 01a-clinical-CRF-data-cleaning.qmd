...
crf7 |> 
  ...
  gt::gt(caption = "Number of participants who meet eligibility criteria")
...
crf8 |> 
  ...
  ggplot() + 
  aes(y = protection_type) + 
  geom_text(
    stat = "count",
    aes(label = ..count..),
    hjust = -0.2,            # déplacement du texte à droite de la barre
    color = "black",
    size = 3
  ) +
  geom_bar(color="darkblue", fill="cornflowerblue") + 
  facet_grid(~site, scale = "free") + 
  labs(y="Protection Type") + 
  scale_x_continuous(expand = expansion(mult = c(0, 0.5)))
```
...
crf9 |> 
  mutate(site = ifelse(str_detect(pid, "06810"), "MGH", "CAP"), 
         metronidazole = case_when(
           str_detect(antibiotic_type, "metronidazole") ~ "metronidazole",
            antibiotic_type %in% c(NA, "none", "do_not_know", "do_not_know, no") ~ "None / unknown",
            TRUE  ~ "No metronidazole"
         )) |> 
  ggplot() + 
  aes(y = antibiotic_type) + 
  geom_bar(color="darkblue", fill="cornflowerblue") + 
  geom_text(
        stat = "count",
        aes(label = ..count..),
  ...
  facet_grid(metronidazole~site, scale = "free") + 
  labs(y="Antibiotic Type") +
  scale_x_continuous(expand = expansion(mult = c(0, 0.5)))
...
crf10 |> 
  mutate(site = ifelse(str_detect(pid, "06810"), "MGH", "CAP")) |> 
  ggplot() + 
  aes(y = sexual_partners_past_month) + 
  geom_text(
    stat = "count",
    aes(label = ..count..),
    hjust = -0.2,            # déplacement du texte à droite de la barre
  ...
  geom_bar(color="darkblue", fill="cornflowerblue") + 
  facet_grid(~site, scale = "free") + 
  labs(y="Sexual partner last month") + 
  scale_x_continuous(expand = expansion(mult = c(0, 0.15))) 
```
...
For metronidazole exposures (based on CRF22), we first assign a value of 2 to participants who did not miss any doses and bring back their prescription container — that is, those with `visit_code` 1100, `tablets_remain` equal to 0, `metro_dose_miss` marked as "None", and `prescription_container` marked as "Yes".  

Then, we review the remaining cases individually by reading the staff comments to approximate when the missed dose(s) were not taken. A variable `comments_mtz` is also created to include notes for specific participants.

Some participants had a visit that occurred in the middle of their MTZ treatment. In these cases, the CRF22 was completed only for the doses taken up to that visit.  
For most of these participants, the CRF was completed again at the following visit, allowing us to determine whether the full course of treatment was taken.  
However, for a few participants, the CRF was not completed at the subsequent visit, so we cannot know whether the remaining doses were taken.  
For these participants, the remaining doses are coded as “no info”.
...
There were a few inconsistency between the CRF23 and CRF32, when this was the case we trust the clinical CRF (CRF23)
...
exposures <- 
  ...
  mutate( 
    comments_mtz = 
      case_when(
        pid == "068100061" ~ "Visit on MTZ day 6; no missed doses; no follow-up visit",
        pid == "068100062" ~ "Visit on MTZ day 5; no missed doses; no follow-up visit",
        pid == "068100063" ~ "Visit on MTZ day 5; no missed doses; no follow-up visit",
        ...
        pid == "068200085" ~ "Missed 1 day (timing unknown); treatment +1 day to compensate",
    ...
    case_when(
      ...
      pid == "068100061" & visit_code %in% c(1001:1005) ~ "2", 
      pid == "068100061" & visit_code %in% c(1006:1007) ~ "no info", 
...
# QC : participant who missed a dose 
...
# inconsistency between CRF23 and CRF32
...
tmp <- 
  ...
  slice_max(study_day) |> 
  mutate(
    n_study_doses_product_crf23 = 
      across(day1:day7, ~ .x == "Unchecked") |> 
      rowSums()
  ) |> 
  select(pid, n_study_doses_product_crf23)

inconsistency <- 
  exposures |> 
  group_by(pid) |> 
  mutate(
    n_study_product_doses_crf32 = sum(study_product, na.rm = TRUE),
  ) |> 
  select(pid, n_study_product_doses_crf32) |> 
  distinct() |> 
  left_join(tmp)

inconsistency |> 
  filter(n_study_product_doses_crf32 != n_study_doses_product_crf23) 
...
exposures <- 
  ...
  mutate(
    comments_studyproduct = case_when(
      pid == "068100036" ~ "3 tablets fell AFTER insertion; no indication of when during treatment.",
      pid == "068100045"~ "Not in CRF23, and no information in CRF32 (no time of insertion)", 
      pid == "068100050"  ~ "Not in CRF23, and only information for 1101-1104 in CRF32", 
      pid == "068100053" ~ "In CRF32, four insertion times are reported, but CRF23 shows only one dose taken and six returned. The participant stopped because she felt sick.", 
      pid == "068100054" ~ "Modify : 13 doses --> 7 doses (start on 1102)",
      pid == "068100063" ~ "Modify : 15 doses --> 7 doses (between 1101 and 1107)",
      pid == "068100058" ~ "Modify : 8 doses --> 7 doses (remove 1003)",
      pid == "068200020" ~ "1 tablet dropped BEFORE insertion (not taken, 6 doses total).\n  Last dose missing in CRF32 and CRF23, but CRF32 comment seems inconsistent.", 
      pid == "068200225"  ~ "1 tablet fell out AFTER insertion (last dose, visit code 1107)",
      pid == "068200028" ~ "3 tablets fell out AFTER insertion; no indication of when during treatment.",
      pid == "068200347" ~ "Only 6 doses in crf32 but no missing doses in crf23 --> add a doses 11001", 
      TRUE ~ NA_character_
    )
  ) |> 
  mutate(study_product = case_when(
    ...
    pid == "068100053" & visit_code %in% c(1101:1102, 1104:1107, 1201:1202) ~ 0,
...
crf33 |> 
  ...
  ) |> gt::gt(caption = "Sexual practices (before cleaning)")
...
crf33 |> 
  dplyr::count(
    sex_past_24hours, penis_vagina, finger_vagina, mouth_vagina, anal, condom_use, lubricant_use
  ) |> gt::gt(caption = "Sexual practices (after cleaning)")

crf33 |> 
  filter(condom_use == "Unknown") |> 
  gt::gt(caption = "Entries with unknown condom and lubricant use")
