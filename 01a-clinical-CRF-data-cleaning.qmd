...
:::callout-note
The purpose of this document is to clean, format, check the quality, and potentially fix the CRF data.

This document loads the raw CRF data (provided as a `R list` - see first section) and returns 2 output lists:

- `crf_raw` which contains one element (a `tibble`) per CRF with the "raw" CRF data (*i.e.*, similar to the initially loaded `R list`) but from which sensitive information such as visit dates, comments, etc., have been removed and from which "data check metadata" documenting quality checks performed by the CAPRISA data team has also been removed. 

- `crf_clean` which also contains one element per CRF, but where the columns have been formatted, cleaned, corrected when needed, and prepared for downstream analyses.
...
We load the raw CRF data shared by Lara Lewis on July 7th 2025.
...
- transform the `vdate` column which is a `character` in the raw data into a `date`
- define a `study_day` variable that provides the relative date from visit V1 (Enrollment visit, day 0) - to do so, we define a table with the V1 date for each participant (see below)
- define a `flag_date` variable that flags visits with potential mistakes in their visit dates (*e.g.*, they occurred before the study started or after the study ended, or, for a given participant, outside their enrollment window).

The flagged dates are then reviewed and corrected when possible.

### Study start and end dates

We define the study start date as "2023-10-19" and the study end date as "2025-02-18".

```{r}

study_start_date <- as.Date("2023-10-19") # Note: the official study start was 2024-02-01 (https://clinicaltrials.gov/study/NCT06135974?term=NCT06135974&rank=1) but a few participants were enrolled before that official study start date 
...
Since we fixed a few dates, we need to re-define the `day_0_dates` table.
...
#### Relative study day of daily visits

We check the relative study days for the daily "visits" (corresponding to home swab collection and daily survey data)
...
In this section, we create an additional table `exposures` that contains the exposure to antibiotics (metronidazole) and study product for each participant at each daily visit.
...
Note: In CRF32, we noticed an error in the `study_day` for participant 068200281 for visit 1106 (8 instead of 13). We are correcting this here.
...
For metronidazole exposures (based on CRF22), we first assign a value of 2 to participants who did not miss any doses — that is, those with `visit_code` 1100, `tablets_remain` equal to 0, `metro_dose_miss` marked as "None", and `prescription_container` marked as "Yes".  
