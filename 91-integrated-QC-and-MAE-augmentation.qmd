...
# qPCR data - comparison of the total 16S in weekly vs daily samples
...
qpcr_16S <- 
  bind_rows(
    mae[["qPCR_daily"]] |> 
      ...
      expand_grid(type = c("orig", "LL3", "LM")),
    mae[["qPCR"]] |> 
      as_tibble() |>
...
tmp <- 
 full_join(
   selected_time_points |> 
     ...
     dplyr::select(type, copies_per_swab_med, pid, study_day, site) |> 
     dplyr::rename(copies_per_swab_med_weekly = copies_per_swab_med),
   selected_time_points |> 
     filter(assay == "daily qPCR")  |> 
     dplyr::select(copies_per_swab_med, pid, study_day, site) |> 
     distinct() |> 
...
tmp |> 
  ggplot() +
  aes(x = copies_per_swab_med_daily, y = copies_per_swab_med_weekly, col = study_day) +
  facet_grid(
    site ~ type, labeller = label_both
    ) +
