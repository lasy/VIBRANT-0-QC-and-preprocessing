...
If the rows are identical (*i.e.*, have same values for all columns except `dfseq`, `visit_code` and `study_day`), we keep only one row. If the lines are not identical, they are left in the crfs and will be examined manually later.
...
Some variables have the same name in different crf. For example `vaginal_bleeding` is present in CRF19 and CRF33. We will rename these variables by adding the name of the CRF to the end of the variable.
