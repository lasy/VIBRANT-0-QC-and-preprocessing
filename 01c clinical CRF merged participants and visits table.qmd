...
data_dir <- get_data_dir() |> str_c("01 Preprocessed and QCed/")
...
participants_crf[["crf101"]] <- 
  ...
  group_by(pid) |>
  summarise(
    across(-c(edu, income_sources), ~ first(.x)),
...
participants_crf[["crf1_101"]] <-
  participants_crf[["crf1_101"]] |>
  filter(if_any(-c(pid, dfseq, site), ~ !is.na(.))) 
...
participants_crf[["crf2_102"]] <-
  participants_crf[["crf2_102"]] |>
  filter(if_any(-c(pid, dfseq), ~ !is.na(.))) 

crf_temp <- 
  participants_crf[["crf2_102"]] |>
  filter(pid %in% pid_temp) |>
  filter(!is.na(monthly_gross_income))

participants_crf[["crf2_102"]] <-
  participants_crf[["crf2_102"]] |>
  filter(!(pid %in% crf_temp$pid)) |> 
  bind_rows(crf_temp)
...
participant_crfs_merged <-
  ...
  reduce(full_join, by = join_by(pid))
...
#   reduce(full_join, by = join_by(uid, pid, visit_code, dfseq, study_day))
...
visits_crfs_merged <-
  ...
  reduce(full_join, by = join_by(pid, visit_code, dfseq, study_day))
...
visits_crfs_merged <- 
  ...
  full_join(
    crf_clean[setdiff(no_studyday, no_dfseq)] |> 
      ...
      reduce(full_join, by = join_by(pid, visit_code, dfseq)),
