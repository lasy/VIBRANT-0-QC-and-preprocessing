...
participants_crf[["crf2"]] <- 
  participants_crf[["crf2"]] |>
  filter(str_starts(pid, "06810")) |> # keep only MGH participants
  group_by(pid) |>
  summarise(
...
participants_crf[["crf102"]] <-
  participants_crf[["crf102"]] |>
  filter(str_starts(pid, "06820")) |> # keep only CAP participants
  group_by(pid) |>
  arrange(-dfseq) |> 
  slice_head(n = 1) |> 
  ungroup()
...
### BV diagnosed and treatment types
...
participant_crfs_merged |>
  mutate(
    ...
    diagnosed_bv = case_when(
      diagnosed_bv == "Yes" ~ "Diagnosed", 
      diagnosed_bv == "No" ~ "Not\ndiagnosed", 
      TRUE~diagnosed_bv
    )
  ) |> 
  ...
  ggh4x::facet_nested(
    cols = vars(site), 
    rows = vars(metronidazole, diagnosed_bv), 
    scales = "free"
  ) +
  #facet_grid(diagnosed_bv~site, scale = "free") + 
...
participant_crfs_merged |>
  ...
  ggplot() + 
  aes(y = antibiotic_type, fill = diagnosed_bv) + 
  geom_bar() + 
  geom_text(
    stat = "count",
    aes(label = ..count..),
...
```
### BV diagnosed and other treatment type 
...
participant_crfs_merged |>
  ...
  ggplot() + 
  aes(y = treatment_type, fill = diagnosed_bv) + 
  geom_bar() + 
  geom_text(
    stat = "count",
    aes(label = ..count..),
...
```{r}
visits_crfs_merged |> 
  select(pid, visit_code, new_sex_partner) |> View()
```
