...
- a `participant_crfs_merged` table: a table with one row per `pid` with "harmonized" participant-invariant data (*i.e.*, with data from CRFs that were filled only once per participant such as Demographics, Elligibility, etc.).

- `visits_crfs_merged` table: a table with one row per `pid` and `visit_code` with "harmonized" time-varying data (*i.e.*, with data from CRFs that were filled at several visits).
...
crf_clean_file <- 
  data_dir |> fs::dir_ls() |> str_subset("01_crf_clean") |> sort(decreasing = TRUE) |> magrittr::extract(1)
load(crf_clean_file, verbose = TRUE)
# crf_clean |> names()
rm(crf_clean_file)

```
This table will merge ("join") the data from the following CRFs which were expected to be filled only once for each participant:
...
Although these CRFs were supposed to be filled only once per participant, some participants have multiple entries in some CRFs.

Here, we create a function which, for each CRF listed above, compares the rows corresponding to duplicated `pid`s. 

- If the rows are identical (*i.e.*, have same values for all columns except `dfseq`, `visit_code` and `study_day`), rows are "merged" (we only keep one of the two rows). 

- If the lines are not identical, they are left as such in the CRFs and will be examined manually later.


In addition, this function removes the `dfseq` and `study_day` variables from these CRFs.
...
A list of CRFs that still contain duplicates is displayed below. We will examine them manually.
...
participants_crf[["crf1_101"]] <-
  participants_crf[["crf1_101"]] |>
  dplyr::filter(if_any(-c(uid, pid, dfseq, visit_code, site, study_day), ~ !is.na(.))) 
...
participants_crf[["crf2_102"]] <-
  participants_crf[["crf2_102"]] |>
  dplyr::filter(if_any(-c(uid, pid, visit_code, dfseq), ~ !is.na(.))) 
...
We check that the number of repeated `pid` per CRF is zero for each CRF. 
...
We now merge all the CRFs into a single table called `participant_crfs_merged`.
...
This table has `r nrow(participant_crfs_merged)` entries (unique participants) and `r ncol(participant_crfs_merged)` variables.
...
visdat::vis_dat(
  participant_crfs_merged, sort_type = FALSE
  ) +
  theme(
...
We call "visit CRFs" any CRFs which was not present in the "participants CRF" list.

We check that each "visit CRF" includes both the `study_day` and `dfseq` variables. Any CRF that does not meet this requirement is excluded from the visit CRF list.

We further remove CRF 29 (Adverse events) and CRF 34 (Protocol deviation) because they does not contain `visit_code`.
...
visits_crf_name <- 
  setdiff(visits_crf_name, no_dfseq) |> 
  setdiff("crf29") |> 
  setdiff("crf34")
...
In CRF14, one `uid` (=068100002_0000) appears four times. Three of these entries are identical, so we keep only one of them.

```{r}
process_crf_duplicates <- function(crf_clean, crf_names) {

  crf_filtered <- 
    crf_clean[names(crf_clean) %in% crf_names]
  ...
  for (name in crf_names) {
    ...
    if (length(dup_uid) > 0) {
      ...
      crf_filtered[[name]] <- data
        
    } else {
      crf_filtered[[name]] <- data #|> select(-c(dfseq, study_day))
    }
...
visits_crf <- process_crf_duplicates(crf_clean, visits_crf_name)

```
For the uid 068100002_0000 in crf14, we keep the one for which we have data for nugent score (the rest is identical)
...
visits_crfs_merged <- 
  visits_crfs_merged |> 
  full_join(
    crf_list_renamed[setdiff(no_studyday, no_dfseq)] |> 
      map(
        function(df) {
...
visits_crfs_merged |> 
  select(any_of(colnames(visits_crfs_merged)[str_detect(colnames(visits_crfs_merged), "\\.x")]))
```

We remove data for whic `hiv_result` is "missing" and for wich we have no other data. 
```{r}
visits_crfs_merged <-
  visits_crfs_merged |> 
...
We export the two merged tables as `Rdata` files.
