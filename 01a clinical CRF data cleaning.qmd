...
# data_dir <- "C:/Users/wautierl/OneDrive - UCL/Documents/Assistanat/SMCS/VIBRANT/VIBRANT data UCLouvain/Data/raw 20240819/" #

# data_dir <- "C:/Users/lvermeren/OneDrive - UCL/VIBRANT clinical data/Data/Data/raw 20250507/" #LV

data_dir <- str_c(get_clinical_data_dir(), "raw 20250507/") # 20250320
  
data_dir |> fs::dir_ls(regexp = "RData")

```

We use the `Rdata` file: `cap068_ecrf_data.Rdata`.

That file contains a list with the data from all CRFs + data dictionaries:

```{r}

load(str_c(data_dir, "cap068_ecrf_data.Rdata"), verbose = TRUE)
...
all_visit_dates |> count(new_flag) |> gt()
  ...
  function(crf){
    for (col_ix in seq_along(crf)) {
      if (is.character(crf[[col_ix]])) {
        ...
        crf[[col_ix]][crf[[col_ix]] == "A"] <- "Missing"
      ...
      if (is.factor(crf[[col_ix]])) {
        ...
        levels(crf[[col_ix]])[levels(crf[[col_ix]]) == "A"] <- "Missing"
...
additional_income_sub <-
  ...
  mutate(additional_income_type = additional_income_type |> fct_infreq())
...
```{r}
#| results: asis
crf5 |> plot_marginal_dist() |> plot_tabs( )
```
:::
...
crf14 |> 
  ...
  count(site, vaginal_ph_greater_than, vaginal_ph_value) |> 
  gt()
...
```{r}
#| results: asis
crf19 |> plot_marginal_dist() |> plot_tabs( )
```
:::
...
crf23 |> 
  select(pid, dfseq, tablets_remain, n_missed_days) |> 
  left_join(crf13 |> select(pid, group4), by = "pid") |> 
  filter(!is.na(tablets_remain)) |> 
  ggplot() +
  aes(x = tablets_remain, y = n_missed_days, col = group4, shape = dfseq |> factor()) +
...
crf23 |> 
  count(
    tablets_remain, used_applicators
  )
...
In this section, we create an additional table `exposures` that will contain the exposure to antibiotics and study product for each participant at each daily visit.


First, we create a `pids` vector containing all the id's of the randomised participants (from crf13). 

Next we create the `visits` vector containing all the visits code (dfseq) of all the daily visits (crf32). 

Then, we create an `exposures` data frame expanding all visit codes for each pid.

We will then add for each daily visit the number of antibiotics (from crf22) and/or study product (from crf23 & crf32) taken by the participant. 

Data from the crf22 are processed manually (i.e., each comments from the study staff is checked to see if the participant took the antibiotics or not).
...
pids <- raw_data_list$cap068_data_13$pid
visits <- raw_data_list$cap068_data_32$dfseq |> unique()
...
# add participants with no missing doses to the table "exposure_antabiotics". Put value of 2 for antabiotics for days 1001->1007
...
exposures <- 
  ...
  mutate(
    metronidazole =  case_when(
      pid %in% pid_temp & dfseq >= 1001 & dfseq <= 1007 ~ "2", 
      TRUE ~ NA_character_)
    )
...
exposures <- 
  exposures |>
  mutate( 
    comments_antibiotics = 
      case_when(
        pid == "068200253" ~ "4 evening doses forgot but we don't knwo when (we put 4 days with 1 doses)", 
        pid == "068200281" ~ "3 doses missed but she don't know when (we put 3 days with 1 doses)", 
        pid == "068200387" ~ "1 dose missed but she don't know when (we put 1 day with 1 dose)",
        pid == "068200375" ~ "one tablet disappeared (3 doses one day ?)", 
        pid == "068200378" ~ "take one 1006 morning, 3 tablet return"
      ),
    metronidazole = 
    case_when(
      pid %in% pid_ok & dfseq %in% c(1001:1007) ~ "2",
      
      pid == "068100006" & dfseq == 1002 ~"1", 
      pid == "068100006" & dfseq %in% c(1001,1003:1007) ~"2",
      pid == "068100011" & dfseq %in% c(1001) ~ "1", # start at night
      pid == "068100011" & dfseq %in% c(1002:1007) ~ "2",
      pid == "068100026" & dfseq %in% c(1001:1003, 1005:1007) ~ "2",
      pid == "068100026" & dfseq == 1004 ~ "1",
      pid == "068100036" & dfseq %in% c(1001:1002,1004:1007) ~ "2",
      pid == "068100036" & dfseq == 1003 ~"1",
      pid == "068100045" & dfseq %in% c(1001:1003, 1005:1007) ~"2",
      pid == "068100045" & dfseq == 1004 ~"1",
      pid == "068100046" & dfseq %in% c(1002:1007) ~"2",
      pid == "068100046" & dfseq == 1001 ~"1",
      pid == "068100062" & dfseq %in% c(1001:1004) ~"2",
      pid == "068100062" & dfseq %in% c(1005:1007) ~ "no info",
      pid == "068100063" & dfseq %in% c(1001:1002,1004) ~"2",
      pid == "068100063" & dfseq == 1003 ~"1",
      pid == "068100063" & dfseq %in% c(1005:1007) ~ "no info",
      pid == "068200020" & dfseq %in% c(1001) ~"1", # start 1001 at night
      pid == "068200020" & dfseq %in% c(1002:1007) ~"2",
      pid == "068200020" & dfseq == 1007 ~ "no info",
      pid == "068200058" & dfseq %in% c(1001:1006) ~"2",
      pid == "068200058" & dfseq == 1007 ~"1",
      pid == "068200058" & dfseq == 1008 ~"1",
      pid == "068200085" & dfseq %in% c(1001:1003, 1005:1007) ~"2",
      pid == "068200085" & dfseq == 1004 ~"0",
      pid == "068200087" & dfseq %in% c(1001, 1004:1005) ~"2",
      pid == "068200087" & dfseq %in% c(1002:1003) ~"1",
      pid == "068200087" & dfseq %in% c(1006:1007) ~ "no info",
      pid == "068200118" & dfseq %in% c(1001,1002,1005:1007) ~"2",
      pid == "068200118" & dfseq == 1003 ~"1",
      pid == "068200118" & dfseq == 1004 ~"0",
      pid == "068200118" & dfseq %in% (1008:1009) ~ "no info",
      pid == "068200150" & dfseq %in% c(1001:1003, 1005:1007) ~"2", 
      pid == "068200150" & dfseq == 1004 ~"1",
      pid == "068200191" & dfseq %in% c(1001:1006) ~"2",
      pid == "068200191" & dfseq == 1007 ~ "no info",
      pid == "068200239" & dfseq %in% c(1001:1006) ~"2",
      pid == "068200239" & dfseq %in% c(1007,1008) ~"1",
      pid == "068200224" & dfseq %in% c(1001:1006) ~"2",
      pid == "068200224" & dfseq == 1007 ~ "no info",
      pid == "068200240" & dfseq %in% c(1001:1006) ~"2",
      pid == "068200240" & dfseq == 1007 ~"1",
      pid == "068200253" & dfseq %in% c(1001:1002) ~"2", 
      pid == "068200253" & dfseq %in% c(1003:1006) ~"1",
      pid == "068200253" & dfseq == 1007 ~ "0",
      pid == "068200281" & dfseq %in% c(1001:1002, 1006) ~"2", 
      pid == "068200281" & dfseq %in% c(1003:1005) ~"1",
      pid == "068200281" & dfseq == 1007 ~ "no info",
      pid == "068200310" & dfseq %in% c(1001:1003,1006) ~"2", 
      pid == "068200310" & dfseq %in% c(1004:1005) ~"1",
      pid == "068200310" & dfseq == 1007 ~ "no info",
      pid == "068200356" & dfseq %in% c(1001:1005) ~"2",
      pid == "068200356" & dfseq %in% c(1006:1007) ~ "no info",
      pid == "068200357" & dfseq %in% c(1001:1004,1006:1007) ~"2",
      pid == "068200357" & dfseq %in% c(1005) ~"1", # 
      pid == "068200366" & dfseq %in% c(1001:1004, 1006:1007) ~"2",
      pid == "068200366" & dfseq == 1005 ~"1",
      pid == "068200375" & dfseq %in% c(1001:1006) ~"2",
      pid == "068200375" & dfseq == 1007 ~"1", 
      pid == "068200378" & dfseq %in% c(1001, 1003, 1004) ~"2",
      pid == "068200378" & dfseq %in% c(1002, 1005) ~"1",
      pid == "068200378" & dfseq %in% c(1006,1007) ~ "2", 
      pid == "068200414" & dfseq %in% c(1001, 1005, 1006) ~"2", 
      pid == "068200414" & dfseq %in% c(1002:1004) ~"1",
      pid == "068200414" & dfseq == 1007 ~ "2",
      pid == "068200428" & dfseq %in% c(1001:1006) ~"2",
      pid == "068200428" & dfseq %in% c(1007:1008) ~"1",
      pid == "068200460" & dfseq %in% c(1001:1005, 1007) ~"2",
      pid == "068200460" & dfseq == 1006 ~"0",
      pid == "068200465" & dfseq %in% c(1002:1004) ~"2",
      pid == "068200465" & dfseq %in% c(1001, 1005) ~"1",
      pid == "068200465" & dfseq %in% c(1006:1007) ~ "2"
      )
  )

```

We do the same for the study product:
...
exposures_studyproduct <- 
  crf32_raw |> 
  select(pid, dfseq, insert_study_product_time) |> 
...
exposures <- 
  ...
  mutate(
    comments_studyproduct = case_when(
      pid == "068100036" ~ "3 PILLS FELL OUT OF HER VAGINA AFTER A FEW HOURS, FULLY INTACT.",
      pid == "068100045" ~ "Not in cfr23", 
      pid == "068100050" ~ "Not in crf23", 
      pid == "068100053" ~ "Report taking 4 time the study product in crf32 but only take 1 (according to crf23)", 
      pid == "068100054" ~ "Modify : 13 doses --> 7 doses (start on 1102, ok ?)",
      pid == "068100063" ~ "Modify : 15 doses --> 7 doses",
      pid == "068100058" ~ "Modify : 8 doses --> 7 doses (remove 1003)",
      pid == "068200020" ~ "6 doses OK but I think 1004 or 1005 is missing and 1102 is not missing --> TO CHECK", 
      pid == "068200347" ~ "Only 6 doses in crf32 but no missign doses in crf23 --> add a doses 11001", 
      TRUE ~ NA_character_
    )
  ) |> 
  mutate(study_product = case_when(
    pid %in% pid_temp & dfseq %in% c(1101:1107) ~ "1",
    pid %in% pid_temp & !(dfseq %in% c(1001:1007)) ~ "0",
    pid == "068100013" & dfseq %in% c(1101:1107) ~ "1", 
    pid == "068100006" & dfseq %in% c(1301, 1401:1407) ~ "0",
    pid == "068100053" & dfseq %in% c(1101:1102, 1104:1107) ~ "0",
    pid == "068100053" & dfseq == 1103 ~"1", 
    pid == "068100054" & dfseq %in% c(1202:1207, 1003,1004) ~"0",
    pid == "068100058" & dfseq == 1003 ~ "0",
    pid == "068100061" & dfseq %in% c(1000:1007, 1201:1207, 1301) ~ "0", 
    pid == "068200310" & dfseq == 1103 ~"1", 
    pid == "068200347" & dfseq == 1101 ~"1",
    pid == "068200278" & dfseq == 1104 ~ "0", 
    pid == "068200281" & dfseq == 1106 ~"0",
    TRUE ~ as.character(study_product)  
  ))
...
exposures |>
  ggplot(aes(y = factor(dfseq), x = factor(pid), color = metronidazole)) +
  geom_point() +
  scale_color_discrete(na.translate = FALSE) +  
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), 
        plot.title = element_text(hjust = 0.5)) + 
  labs(title = "Exposures metronidazole", y = "Pid", x = "dfseq")
...
exposures |>
  ggplot(aes(y = factor(dfseq), x = factor(pid), color = study_product)) +
  geom_point() +
  scale_color_discrete(na.translate = FALSE) +  
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), 
        plot.title = element_text(hjust = 0.5)) + 
  labs(title = "Exposures study product", y = "Pid", x = "dfseq")
...
```{r}
#| results: asis
crf26 |> plot_marginal_dist() |> plot_tabs( )
```
:::
...
# crf27 |> 
#   mutate(
#     specimen_date_parsed = as.Date(specimen_collection_date, format = "%d%b%y")
#   ) |> 
#   filter(
#     !(is.na(vdate) & is.na(specimen_date_parsed)) &  
#     vdate != specimen_date_parsed                    
#   ) |> 
#   select(pid, vdate, specimen_collection_date, specimen_date_parsed)

# visit dates are correct - these are typos in the specimen collection dates
...
# Questionnaire pour plusieurs visites: vdate
...
# vaginal_swab_sub <-
#   crf35 |>
#   select(pid, dfseq, vaginal_swab, vaginal_swab_sti_testing, vaginal_swab_amsel) |>
#   pivot_longer(cols = -c(pid, dfseq), names_to = "vaginal_swab_type", values_to = "vaginal_swab") |>
#   mutate(vaginal_swab = (tolower(vaginal_swab) == "checked")) |>
#   group_by(pid, dfseq) |>
#   summarize(vaginal_swab_type = str_c(vaginal_swab_type[vaginal_swab], collapse = ", ")) |>
#   mutate(vaginal_swab_type = vaginal_swab_type |> fct_infreq())

# swab_sub <-
#   crf35 |>
#   select(pid, dfseq, pap_smear, cervicovaginal_hpv, endocervical_cytobrush, rectal_swab, urine, urine_urinalysis, urine_poc_pregnancy, 
#          blood_sample, blood_hiv, blood_syphilis, blood_hsv, blood_count, blood_type, blood_research_tube, 
#          other_specimen1, specimen1_specify, other_specimen2, specimen2_specify) |>
#   pivot_longer(cols = -c(pid, dfseq), names_to = "swab_type", values_to = "swab") |>
#   mutate(swab = (tolower(swab) == "checked")) |>
#   group_by(pid, dfseq) |>
#   summarize(swab_type = str_c(swab_type[swab], collapse = ", ")) |>
#   mutate(swab_type = swab_type |> fct_infreq())

# crf35 <-
#   crf35 |>
#   select(pid, dfseq, study_day, softcup, softcup_time_placed, softcup_time_removed) |>
#   left_join(vaginal_swab_sub, by = c("pid", "dfseq")) |>
#   left_join(crf35|>select(pid, dfseq, self_collected_swabs), by = c("pid", "dfseq")) |>
#   left_join(swab_sub, by = c("pid", "dfseq")) |>
#   left_join(crf35|>select(pid, dfseq, home_swabs), by = c("pid", "dfseq"))
...
crf35 |> 
  ...
  count(softcup, softcup_time_placed_cat, softcup_time_removed_cat) |> 
  arrange(-n)


crf35 |> 
  count(vaginal_swab, vaginal_swab_sti_testing, vaginal_swab_amsel)

crf35 |> 
  count(self_collected_swabs)
...
crf42 |> 
  ggplot() +
  aes(x = dfseq |> as.character(), y = pid |> factor(), col = missed_visits_reasons) +
  geom_point()
```
...
crf45 |> 
  ggplot() +
  aes(x = dfseq |> factor(), y = pid |> factor(), color = psa_results) +
  geom_point() +
  scale_color_manual(
    "PSA results",
...
crf46 <-
  crf46 |>
  rename(negative_control = negative_ontrol)
...
crf46 |> 
  ...
  full_join(
    ...
    by = join_by(pid)) |>
  count(total_applicators_stained, used_applicators, tablets_remain)
...
It looks like the home swab visit codes might be off for some participants. And it also looks like this manifest wasn't filled systematically in the US.

We'll use this CRF to check if any of the home swab visit codes needs to be changed when loading the daily swab 16S rRNA seq data.

```{r}

# crf47_long |> 
#   filter(!is.na(home_swab_visit_code)) |> 
#   group_by(pid, crf_visit_code) |> 
#   mutate(number_of_home_swab_visit_codes = n()) |> 
#   ungroup() |> 
#   write_csv("CRF47_home_swab_visit_codes.csv")
...
# dir <- "/Users/lvermeren/OneDrive - UCL/VIBRANT clinical data/Data processed/" #LV 

dir <- get_data_dir() |> str_c("01 Preprocessed and QCed/")
