...
# actually 6 replicates instead of 3 for all targets
# qpcr |> 
#   dplyr::filter(!is.na(VIBR_Sample_ID)) |>
#   dplyr::count(VIBR_Sample_ID, gDNA_Plate_ID, Target) |> 
#   dplyr::filter(n > 3)

qpcr |> 
  dplyr::filter(Target == "16S") |> 
  dplyr::filter(VIBR_Sample_ID %in% c("2286665", "2292179")) |> 
  select(VIBR_Sample_ID, Data_row, Well, Sample, Cq, Copies_per_swab, gDNA_Plate_ID, gDNA_Plate_Well)
...
# TODO: no samples in redo: document that there are none, then remove the column
```
...
tmp |> dplyr::count(n_plates, any_in_redos) # *Almost* all samples's replicates are on a single plate
# TODO: check what are the samples that are on several plates

tmp |> dplyr::count(n_replicates, n_plates, any_in_redos) # All samples have at least 3 replicates

```

We note that almost all samples's replicates are on a single extraction plate (`ext_lib_plate_nb` or `ext_lib_plate_id`); and all samples have at least 3 replicates. A few samples are on two plates and have 6 replicates.
