...
There are `r nrow(samples_without_entries_in_mg_manifest)` clinical or vibrant positive/negative control samples without a matching entry in the metagenomics manifest.
...
There are `r mg_samples |> filter(is.na(qpcr_sample_type)) |> nrow()` samples for which we have metagenomics data but no qPCR data.
...
purrr::walk(
  ...
  \(x) {
    ...
    g <- 
      qpcr |> 
      filter(qpcr_sample_type %in% "Standard", target == x) |>
