...
- a `participants` table with study-level participant-invariant information (arm, site, etc.), including the population flags along with a 

  + `participants_variable_dictionary` which provides a description for each of the columns in the `participants` table
...
## Randomization (`randomized` and `treatment_code`)

Randomization data is available in CRF13. 


:::callout-note
Since data QC happened before unblinding, we do not know the study arm of participants at this stage. The only open (non-blinded) arm was the "LC-106-o" arm in which participants received the LBP before they completed their MTZ treatment. Participants in that arm are identified with the column `group4` since that arm was the fourth arm of the trial. 

Unblinding happened on June 17, 2025, after the `MultiAssayExperiment` object was created, and arms were added to the `MultiAssayExperiment` object in the script `92-unblinding.qmd`.
:::

Here, we simply flag if participants were randomized or not (i.e., whether they had a `treatment_code`), if they were part of the overal arm (LC-106-o arm - `group4`), and we create a placeholder `arm` column that will be updated after unblinding.
...
These flags were checked with Lara Lewis (CAPRISA statistician) and validated.
...
We export the `participants`, `visits_long`, and `visits` tables along with their variable dictionaries.
