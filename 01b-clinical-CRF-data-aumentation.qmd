...
- a `participants` table with study-level participant-invariant information (*e.g.*, arm, site, etc.), including the population flags along with a 

  + `participants_variable_dictionary`, a table describing the variables contained in the `participants` table
...
# Loading the CRF data processed in the previous step (`01a clinical CRF data cleaning.qmd`)
...
Randomization data is available in CRF13. 

:::callout-important
This code, along with all assay QCs, was written and executed **before** arm unblinding, which happened on June 17th, 2025. Consequently, here, we only have data related to *whether* a participant was randomized (`randomized`), and if so, whether they were in the only open (unblinded) arm (`group4`, arm LC-106-o in which participants received the LBP before finishing their course of metronidazole), and what their randomization code (`treatment_code`) was, but we do not yet know the actual arm of the participant (`arm`). We still define a placeholder `arm` column that will be updated after unblinding. 

Unblinding itself is done in the script `92-unblinding.qmd`, after all data QC has been performed and that a `MultiAssayExperiment` object has been created.
:::


We look at CRF13 to identify randomized participants: randomized participants are participants with a `treatment_code`.
...
We then check if the participants have been replaced (CRF44) or met any of the replacement criteria (CFR33 for bleeding, CRF20 for antibiotics, CRF15 for HSIL pap).
