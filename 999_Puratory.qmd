...
qpcr |> 
  ...
  ggplot() +
  aes(x = starting_quantity, y = copies_per_swab, col = replicate_nb |> factor()) +
  geom_abline(slope = 1, intercept = 0, col = "gray") +
  geom_point(alpha = 0.5, size = 0.5) +
  facet_grid(vmrc_group + target ~ ext_lib_plate_nb) +
  scale_x_log10() +
  scale_y_log10() +
  theme(
...
This looks "perfect" - I assume the `copies_per_swab` values are the expected ones.




### Starting quantity, adjusted quantity, and dilution factor
...
qpcr |>
  filter(vmrc_group %in% c("VMRC_1", "VMRC_2")) |>
  ggplot() +
  aes(
    x = well_row |> factor(), 
    y = well_col |> fct_rev(), 
    fill = starting_quantity_sq |> log10()
    ) +
  geom_tile() +
  facet_grid(vmrc_group + target ~ ext_lib_plate_nb, scales = "free", space = "free") +
  xlab("Well column") + ylab("Well row") 
...
qpcr |> 
  ...
  # filter(pid == pid[1]) |>
  select(n_target_detected, pid, visit_code, target, ext_lib_plate_nb, copies_per_swab_adj_median) |> 
  arrange(pid, target, visit_code) |> 
  distinct() |> 
  ggplot() +
  aes(x = visit_code, y = copies_per_swab_adj_median |> asinh(), fill = ext_lib_plate_nb |> factor()) +
  geom_col() +
  facet_grid(pid ~ ., scale = "free") +
  theme(
    legend.position = "bottom",
...
## Participants with detected LBP strains at the screening visits
...
qpcr |> 
  filter(
    ...
    sample_category %in% c("Expected sample"),
    visit_code == 1000,
    copies_per_swab_adj_median > 0
    ) |> 
  select(pid, location, strain_origin, vmrc_group, target, copies_per_swab_adj_median) |> 
  distinct() |> 
  ggplot() +
  aes(x = pid, y = copies_per_swab_adj_median, fill = str_c(vmrc_group, " - ", target)) +
  geom_col() +
  scale_y_log10() +
  scale_fill_discrete("VMRC group - Target") +
  facet_grid(strain_origin ~ location, scales = "free", labeller = label_both) +
  theme(
...
qpcr |> 
  ...
  select(pid, location, ext_lib_plate_nb, strain_origin, vmrc_group, target, copies_per_swab_adj_median) |> 
  distinct() |> 
  ggplot() +
  aes(x = pid, y = copies_per_swab_adj_median, fill = ext_lib_plate_nb |> factor()) +
  geom_col() +
  scale_y_log10() +
  scale_fill_discrete("VMRC group - Target") +
  facet_grid(strain_origin ~ location, scales = "free", labeller = label_both) +
