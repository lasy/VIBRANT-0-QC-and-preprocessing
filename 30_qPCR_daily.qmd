...
```{r}
qpcr |> dplyr::filter(Target == "16S") |>  dplyr::filter(!is.na(VIBR_Sample_ID)) |> dplyr::count(VIBR_Sample_ID, gDNA_Plate_ID ) |> dplyr::filter(n > 3)

qpcr |> dplyr::filter(Target == "16S") |> dplyr::filter(VIBR_Sample_ID %in% c("2286665", "2292179")) |> select(Data_row, Well, Sample, Cq, Copies_per_swab)
...
Should I use the same technique to check for labeling errors for the 6 replicates?
:::
...
manifest <- 
  ...
  as.data.frame() |> 
  as_tibble() |> 
  select(sample_id, sample_id_barcode, pid, visit_code, sample_type, control_type, randomized, location) |> 
  distinct() |>
