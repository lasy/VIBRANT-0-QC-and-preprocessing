...
### Number of replicates per VIBRANT sample on the gDNA plates

We first check for any replicates on the extraction plates (i.e., `vibr_sample_id` that are found twice in `ext_lib_plate_nb` or `ext_lib_plate_id`). 
...
tmp <- 
  ...
  select(vibr_sample_id, ext_lib_plate_nb, ext_lib_plate_id, ext_lib_position) |> 
  distinct() |> 
  group_by(vibr_sample_id, ext_lib_plate_nb, ext_lib_plate_id) |> 
  mutate(n_replicates_on_ext_plates = n()) |> 
  ungroup()

```

There are `r tmp$vibr_sample_id[tmp$n_replicates_on_ext_plates > 1] |> unique() |> length()` samples that have replicates on the extraction plates:

```{r}

tmp |> 
  filter(n_replicates_on_ext_plates > 1) |> 
  arrange(-n_replicates_on_ext_plates, ext_lib_plate_nb) |> 
  group_by(vibr_sample_id) |> 
  gt(
    caption = "Sample IDs replicated in the extraction plates", 
...
We change the `vibr_sample_id` of these samples with replicates in the gDNA (extraction) by adding "`_1`" and "`_2`" to their `vibr_sample_id`
...
qpcr <-
  ...
  left_join(
    qpcr |>
      filter(!is.na(ext_lib_plate_id), !is.na(ext_lib_position)) |>
      distinct(vibr_sample_id, ext_lib_plate_nb, ext_lib_plate_id, ext_lib_position) |>
      group_by(vibr_sample_id, ext_lib_plate_nb, ext_lib_plate_id) |>
      arrange(ext_lib_plate_id, ext_lib_position, .by_group = TRUE) |>
      mutate(n_pos = n()) |>
      filter(n_pos == 2) |>                               
      mutate(vibr_sample_id_new = str_c(vibr_sample_id, "_", row_number())) |>
      ungroup() |>
      select(vibr_sample_id, ext_lib_plate_id, ext_lib_position, vibr_sample_id_new),
    by = c("vibr_sample_id", "ext_lib_plate_id", "ext_lib_position")
  ) |>
  mutate(vibr_sample_id = coalesce(vibr_sample_id_new, vibr_sample_id)) |>
  select(-vibr_sample_id_new)

```

We check for unicity of `vibr_sample_id`s in the extraction plates again:
...
if (nrow(tmp) > 0){
  tmp |> 
  gt(caption = "Remaining sample IDs replicated in the extraction plates")
}
...
:::callout-note
Check with Michael and/or Sinaye about these two samples, and check in the 16S amplicon sequencing manifest if we also find these replicates 
:::


>>> TODO: document the samples that had a position on the gDNA plates BUT did NOT have a `vibr_sample_id` (i.e., `NA` in the `vibr_sample_id` column).
>>> TODO: show the gDNA plate design with whether `vibr_sample_id` was missing.

>>> TODO: check later with the manifest data


>>> LIKELY EXPLANATION: LAZYNESS
> Les colones "vibr_sample_id" didn't get filled for some of the plates -> we can likely fill in based on the plates for which we have the data....
...
manifest <- 
  ...
  as_tibble() |> 
  select(
    sample_id, sample_id_2, sample_id_barcode, specimen_type, well_id, pool, 
    original_mgh_aliquot_unique_identifier, ext_lib_plate_type, 
    ext_lib_plate_nb, location,
    pid, visit_code, sample_type, control_type, randomized, location) |> 
  distinct()
...
### Number of replicates per VIBRANT sample on the PCR plates

We now check how many replicates each `vibr_sample_id` has on the PCR plates (*i.e.*, replicates per `target`).
...
tmp <- 
 qpcr |> 
  dplyr::filter(!is.na(vibr_sample_id)) |> 
  select(vibr_sample_id, target, pcr_plate_id, well) |> 
  distinct() |> 
  group_by(vibr_sample_id, target) |> 
  mutate(
    n_replicates_per_target = n(),
    n_plates = pcr_plate_id |> unique() |> length()
    ) |> 
  ungroup() 

tmp |>
  select(vibr_sample_id, n_replicates_per_target, n_plates) |> 
  distinct() |> 
  dplyr::count(n_replicates_per_target, n_plates, name = "n_samples") |> 
  gt(
    caption = "Number of qPCR replicates per VIBRANT sample and target"
  )

```

Most samples have 3 replicates. But a few samples have 6 replicates located on 2 different plates:
...
tmp <- 
 qpcr |> 
  ...
  select(vibr_sample_id, target, pcr_plate_id, well) |> 
  distinct() |> 
  group_by(vibr_sample_id, target, pcr_plate_id) |> 
  summarize(
    n_replicates_per_target = n(),
    .groups = "drop"
    ) |> 
  group_by(vibr_sample_id) |> 
  mutate(n_plates = pcr_plate_id |> unique() |> length()) |> 
  ungroup() |> 
  arrange(vibr_sample_id, target, pcr_plate_id)
...
tmp |> 
  filter(n_plates > 1) |> 
  ggplot() +
  aes(x = target, y = pcr_plate_id |> fct_rev(), fill = pcr_plate_id) +
  facet_grid(vibr_sample_id ~ ., scales = "free", space = "free") +
  geom_tile(color = "white") +
  geom_text(aes(label = n_replicates_per_target), col = "black") +
  ylab("qPCR plate name") +
  scale_fill_brewer("qPCR plate", type = "qual", palette = 7) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
  
```


All samples that had more than 3 replicates had exactly 6 replicates, split in two plates with 3 replicates per plate, and the 2nd plate what plate 8.

So, we assume that these samples may correspond to samples who failed QC in their first run and got selected for another qPCR round.

We'll flag them so that we can check their QC values later.

```{r}

qpcr <- 
  qpcr |> 
  select(-any_of(c("n_plates", "qpcr_redo"))) |> 
  left_join(
    tmp |> 
      select(vibr_sample_id, target, pcr_plate_id, n_plates) |> 
      mutate(qpcr_redo = n_plates > 1),
    by = join_by(pcr_plate_id, target, vibr_sample_id)
  ) 
...
In this section, we define a `qpcr_sample_type` variable to define the type of sample (Standard, Water, Negative Controls, Clinical sample, etc.) that was placed in each well, and a `qpcr_sample_id` variable to define unique sample IDs that is valid across sample types. 

#### Standards

First, we harmonized standard sample names since the 16S and LBP strain plates use different notations: 

-   `content` take value from "Std-01" to "Std-07" for the 16S (where the number correspond to the number of replicates) and "Std" for LBP. 
-   `sample` take vale from "std 10^1" (when content == Std-07) to "std 10^7" (when content == Std-01) for 16S and "Standard 1" to "Standard 6" for LBP.

We harmonized `content` to "Std" and `sample` to "Standard". 

```{r}

# est ce que je garde les numero pour les standard LBP ? 
qpcr <-  
  qpcr |> 
  mutate(
    content_new = 
...
qpcr |> 
  ...
  dplyr::count(content, content_new, starting_quantity) 
...
We now define the `qpcr_sample_type` variable, which is constructed from the information of the `vibr_sample_id` and `sample` columns.
  ...
  mutate(
    qpcr_sample_type = 
      case_when(
        ...
        is.na(vibr_sample_id) & str_detect(content, "NTC") ~ "NTC (No template control)",
        # str_detect(vibr_sample_id, "EQ") ~ "Control sample", # does not exist in this data
        # str_detect(vibr_sample_id, "empty") ~ "Empty", # does not exist in this data
        is.na(vibr_sample_id) & is.na(ext_lib_plate_id) ~ "Unclear ???",
        (is.na(vibr_sample_id) & !is.na(ext_lib_plate_id)) ~ "Water or empty",
...
#### qPCR plate wells (coordinates for plotting plate design)
  ...
  mutate(
    qpcr_sample_id = 
      case_when(
        ...
        (qpcr_sample_type == "NTC (No template control)") ~ 
          str_c("ntc_plate_", 
        ...
        (qpcr_sample_type == "Water or empty") ~ 
          str_c("water_or_empty_plate_", 
                ext_lib_plate_nb |> str_pad(width = 2, pad = 0),"_", sample),
        (qpcr_sample_type == "Unclear ???") ~ 
          str_c("unclear_plate_", 
                ext_lib_plate_nb |> str_pad(width = 2, pad = 0),"_", sample),
...
qpcr_sample_type_colors <- c(
  ...
  "Water" = "lightblue1",
  "Empty" = "gray",
  "NTC (No template control)" = "gray50",
  "Unclear ???" = "red",
...
qpcr |> 
  ...
  select(pcr_plate_id, ext_lib_plate_nb, target, well_col, well_row, qpcr_sample_type, sample) |>
  distinct() |>
  ggplot() +
  aes(x =  well_row |> factor(), y = well_col |> factor() |> fct_rev(), fill = qpcr_sample_type) +
  facet_wrap(target ~ pcr_plate_id) +
  geom_tile() +
...
qpcr |> 
  ...
  select(pcr_plate_id, ext_lib_plate_nb, strain_group_qpcr, target, well_col, well_row, qpcr_sample_type, sample) |>
  distinct() |>
  ggplot() +
  aes(x =  well_row |> factor(), y = well_col |> factor() |> fct_rev(), fill = qpcr_sample_type) +
  facet_grid(pcr_plate_id ~ strain_group_qpcr + target) +
  geom_tile() +
  geom_path(aes(group = sample), col = "black", alpha = 0.5) +
  geom_point() +
  scale_fill_manual(values = qpcr_sample_type_colors) +
...
We merge the qPCR data with the 16S manifest data to add the participant ID (`pid`) and (`visit_code`).
