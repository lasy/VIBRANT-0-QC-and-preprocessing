...
There were a few daily swabs with daily `visit_code` that did not have CRF data, and consequently did not have study days assigned.
...
mae_sample_map |> 
  ...
  geom_tile() +
  facet_grid(
    location + ifelse(randomized, "Randomized", "Not randomized")  + arm  ~ visit, 
    scales = "free_y", space = "free_y"
    ) + # sample_category
  scale_y_discrete("Participants IDs") + #, breaks = "NULL")  +
  xlab("") +
